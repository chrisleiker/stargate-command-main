...
const { writeAppList } = require('./app-list');
const { scanLinuxApps } = require('./linux-apps');
...
function scanMacApps() {
  ...
  function visit(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && lower.endsWith('.app')) {
        const bundleName = entry.name.replace(/\.app$/i, '');
        const key = normalizeName(bundleName);
